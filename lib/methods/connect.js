...
const connect = server => {
  ...
  const keepAlive = options => {
    const onPoolError = async error => {
      /*
        Unclear what sort of error should trigger a reconnection. My tests
        with docker always send message:
        terminating connection due to unexpected postmaster exit
        but maybe any fatal error here is appropriate to try?
      */

      if (pgConnectionRetry.retries && error.severity === 'FATAL') {
        server.log(
          [pkg.name, 'error'],
