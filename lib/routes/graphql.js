...
const onPreResponse = server => {
  ...
  return async (request, h) => {
    ...
    if (operationName === clearTokenOperationName) {
      server.log([pkg.name, 'debug'], `clearing token from ${operationName}`);
    ...
    if (refreshTokenQuery && request.auth.isAuthenticated) {
      server.log([pkg.name, 'debug'], 'testing if token is stale');
      ...
      if (shouldRefresh) {
        server.log([pkg.name, 'debug'], 'refreshing stale token');
        ...
        try {
          ...
          server.log([pkg.name, 'debug'], 'stating refreshed token');
    ...
    if (operationName === getTokenOperationName) {
      server.log([pkg.name, 'debug'], `setting token from ${operationName}`);
    ...
    if (operationName === refreshTokenOperationName) {
      server.log([pkg.name, 'debug'], `refreshing token from ${operationName}`);
...
const handler = server => {
  ...
  return async request => {
    ...
    if (
      ...
      server.log([pkg.name, 'debug'], `cached query for ${logOp}`);
      ...
      server.log([pkg.name, 'debug'], `query for ${logOp}`);

      result = await postgraphile.performQuery(payload, pre);
    }

    server.log([pkg.name, 'debug'], `${logOp} succeeded`);
