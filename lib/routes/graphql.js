...
const getJwtToken = server => {
  ...
  return (request, h) => {
    if (cookieAuthentication) {
      const token = request.state[cookieAuthentication.key];
      if (token) {
        return token;
      }
    }
...
const handler = server => {
  ...
  const allowedCacheOperation = has(cacheAllowedOperations);

  return async request => {
    const {payload, pre} = request;
    const {operationName} = payload;

    if (operationName && allowedCacheOperation(operationName)) {
